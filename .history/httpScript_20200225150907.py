...
class ClusterHostOnboarder():
  ...
  def postStorages(self):
    ...
    with open('./payloads/storage.json') as json_file:
      storagePayload = json.load(json_file)

      for dataStore in ['DS1', 'DS2']:
        storagePayload['name'] = self.resourcePoolName + dataStore
        self.storages.append(self.postToOpaas('storage', storagePayload))
